...
class Solution(object):
    def _bonus(self, n, leadership, operations):
        """
        :type n: int
        ...
        # Time Limit
        leaders = {}
        for leader, staff in leadership:
            if leader not in leaders:
                leaders[leader] = TreeNode(leader)
            if staff not in leaders:
                leaders[staff] = TreeNode(staff)
            leaders[leader].children.append(staff)

        result = []
        for operation in operations:
            if operation[0] == 1:
                leaders[operation[1]].value += operation[2]
            elif operation[0] == 2:
                self.append_value(operation[1], operation[2], leaders)
            else:
                result.append(self.count_value(operation[1], leaders))
    ...
    def bonus(self, n, leadership, operations):
        ...
        # Still Wrong Answer
        # Change Solutions
        # 1. 两种加法都别忘了父节点，都需要更新父节点
        # 2. 需要将结果取余，如果实时取余也会超时，在最终结果处取余
        # 3. 树结构需要用递归和类，消耗较大，用数组实现会快一些
        leaders = {}
        for leader, staff in leadership:
            if leader not in leaders:
                leaders[leader] = TreeNode(leader)
            if staff not in leaders:
                leaders[staff] = TreeNode(staff)
            leaders[leader].children.append(staff)
            leaders[staff].parent = leaders[leader]

        self.count_son_sum(1, leaders)

        result = []
        for operation in operations:
            if operation[0] == 1:
                leaders[operation[1]].value += operation[2]
                self.add_parent_sum(operation[1], operation[2], leaders)
            elif operation[0] == 2:
                self.new_append_value(operation[1], operation[2], leaders)
                self.add_parent_sum(operation[1], (leaders[operation[1]].son_sum + 1) * operation[2], leaders)
            else:
                result.append(leaders[operation[1]].value % 1000000007)

        return result

    def append_value(self, leader, value, leaders):
        leaders[leader].value += value
        for child in leaders[leader].children:
            self.append_value(child, value, leaders)

    def new_append_value(self, leader, value, leaders):
        leaders[leader].value += (leaders[leader].son_sum + 1) * value
        for child in leaders[leader].children:
            self.new_append_value(child, value, leaders)

    def count_value(self, leader, leaders):
        value = leaders[leader].value
        for child in leaders[leader].children:
            value += self.count_value(child, leaders)
        return value

    def count_son_sum(self, leader, leaders):
        son_sum = len(leaders[leader].children)
        for child in leaders[leader].children:
            son_sum += self.count_son_sum(child, leaders)
        leaders[leader].son_sum = son_sum
        return son_sum

    def add_parent_sum(self, leader, value, leaders):
        if leaders[leader].parent:
            leaders[leader].parent.value += value
            self.add_parent_sum(leaders[leader].parent.index, value, leaders)


if __name__ == '__main__':
    s = Solution()
    print s.bonus(6, [[1, 2], [1, 6], [2, 3], [2, 5], [1, 4]], [[1, 1, 500], [2, 2, 50], [3, 1], [2, 6, 15], [3, 1], [3,2]])
